      ...
      let option = {
        ...
        tooltip: { // 提示框组件 坐标轴指示器是指示坐标轴当前刻度的工具
          ...
          formatter: function (param) {
            let param2
            // todo 这里要不要改呢？
            // 原来对比的是简体中文的`成交量`,现在改成对比国际化的了，不知道正确不正确
            ...
            if (param && param2) {
              return [
                '<span style="font-size:16px;font-weight:bold;margin-right:10px;">' + param.seriesName +
                '</span>' + param.name + '<hr size=1 style="margin: 3px 0">',
                // '时间：' + param.name +'</br>',
                this.$t('choice.price') + ':' + '<span class="' + classFont + '" style="font-size:16px;">' + param.value + '</span>' +
                '<br/>',
                this.$t('choice.chg') + ':' + '<span class="' + classFont + '" style="font-size:16px;">' + data.rates[param
                  .dataIndex] + '%</span>' + '<br/>',
                this.$t('line.vol') + '(' + this.$t('order.lot') + ')' + ':' + param2.value[0] + '<br/>',
                this.$t('line.tran') + '(' + this.$t('line.thou') + '):' + (data.amounts[param.dataIndex] / 10000).toFixed(2) + '<br/>'
