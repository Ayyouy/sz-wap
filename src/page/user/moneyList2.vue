...
export default {
  ...
  watch: {
    selectedNumber (val) {
      console.log('2selectedNumber', val)
      if (val === '2') {
        this.loaded = false
        this.list = []
        this.pageNum = 1
  ...
  mounted () {
    console.log('List2：什么时候执行呢？', this.selectedNumber, this.selectedNumber === '2', this.selectedNumber === 2)
    if (this.selectedNumber === '2') {
      console.log('List2-2：什么时候执行呢？', this.selectedNumber)
      this.loaded = false
      this.list = []
      this.pageNum = 1
      this.total = 0
