      ...
      >
        <el-form-item label="转账类型">
          <el-radio-group v-model="searchForm.transferType">
            ...
            <el-radio label="1" border>转入</el-radio>
            <el-radio label="0" border>转出</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item label="电话">
          <el-input v-model="searchForm.phoneNum" placeholder="请输入" />
        </el-form-item>
  ...
  methods: {
   async getList() {
    let opt = {
      ...
      let res = await api.transferMoneyList(opt)
      if (res.status === 0) {
        this.list = res.data
      } else {
        Toast(data.msg)
